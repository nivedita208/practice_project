    ...
    refresh: function(frm) {
       // Don't show button if document not saved
        ...
        frm.add_custom_button(__("Create Estimation Request"), function() {
            frappe.confirm(
                'Are u sure u want to create estimation request?',
                function() {
                     // check if scope field is filled
                    if (!frm.doc.custom_scope) {
                        frappe.msgprint({
                            message: __("Please enter the scope before creating Estimation."),
                            indicator: "red"
                        });
                        return; // stop execution
                    }
                    frappe.call({
                        method: "practice_project.custom_code.estimation_request.create_estimation_request", // python method
                        args: {
                            opportunity: frm.doc.name,
                            scope: frm.doc.custom_scope,
                            
                        },
                        callback: function(r) {
                            if (!r.exc) {
                                frappe.msgprint("Estimation Request created successfully!");
                                frm.reload_doc();
                            }
                        }
                    });
                ...
                function() {
                    frappe.msgprint("kitty Remainder:Hare Krishna!! Hare Rama")
                }
            )
        })
    }
